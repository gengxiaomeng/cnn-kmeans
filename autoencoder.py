...
def CreateAutoEncoder():
    ...
    x = layers.Dense(64)(x)
    encoder_output = layers.Dense(32)(x)
   
    encoder = keras.Model(encoder_input, encoder_output, name='encoder')
    
    x = layers.Dense(32)(encoder_output)
    x = layers.Dense(64)(x)
    x = layers.Dense(128)(x)
    x = layers.Dense(196)(x)
    x = layers.Reshape(target_shape=(7,7,4))(x)
...
def TrainAutoencoder(train_images, test_images, y_test, batch_size):
    ...
    # Train autoencoder
    history = autoencoder.fit(train_images, train_images, epochs=10, batch_size=batch_size, verbose=1)
